...
public class DBHelper3 extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE record3 (_id INTEGER PRIMARY KEY AUTOINCREMENT," +
                "   date TEXT, name TEXT, factor INTEGER);");
        String before = settingshared.getString("before3", "설치전");
        sql=String.format("INSERT INTO record3 VALUES(NULL,'00:00','%s',0);",before);
        db.execSQL(sql);
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL("DROP TABLE IF EXISTS record3");
        onCreate(db);
    }
